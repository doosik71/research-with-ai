...
논문의 원문은 https://ar5iv.labs.arxiv.org/html/{arxiv_id} 에서 HTML 형태로 fetch하여 얻을 수 있다.
...
보고서는 논문의 원문을 읽지 않고도 내용을 이해할 수 있도록 충분히 상세해야 한다.
...
보고서의 맨 마지막 행에는 다음과 같이 메타데이터 블록을 추가한다.

{"title": "<논문 제목>", "author": "<저자 목록>", "year": <출판 연도>, "url": "<Arxiv URL>", "summary": "<요약 파일 이름>", "slide": ""}

여기에서 `summary` 필드 값 <요약 파일 이름>은 논문 제목에 다음의 규칙을 적용하여 생성한다.
