...
$$ \min_{\theta} \sum_{t=0}^{T} E_{(x,y)\sim D_t} [L_{CE}(y,f(x;\theta))] $$
...
2. **리허설(Rehearsal)**:
   - 버퍼에서 샘플 $(x', y')$을 무작위로 추출하여 과거 태스크에 대한 모델의 기억을 강화하는 리허설 손실 $L_R$을 계산합니다.
     $$ L_R = E_{(x',y')\sim M_{\tau_{t-1}}} [L_{CE}(y',f(x';\theta_{\tau_{t-1}}))] $$
3. **증류(Distillation)**:
   ...
   - 버퍼에서 샘플 $(x'', z'')$을 무작위로 추출하여 증류 손실 $L_D$를 계산합니다. 이는 현재 모델의 응답이 이전 Logit과 일관성을 유지하도록 보장합니다.
     $$ L_D = E_{(x'',z'')\sim M_{\tau_{t-1}}} [L_{MSE}(z'',h(x'';\theta_{\tau_{t-1}}))] $$
