...
- pruning 없이도 task별 파라미터를 구조적으로 분리할 수 있는가?
...
- **StackNet**
  - 새 task가 들어오면 새 파라미터 구간을 쌓아 올린다.
- **Index module**
  - 입력이 어느 task에서 왔는지 추정한다.
...
- simple but strong dynamic architecture baseline이다.
...
- index module은 task routing을 담당한다.
...
- 이 논문의 본질은
  **"안 잊게 정규화하자"가 아니라 "애초에 task별 파라미터를 분리하자"**다.
