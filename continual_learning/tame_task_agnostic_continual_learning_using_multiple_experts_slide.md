...
- task label 없이 continual learning을 실제로 수행할 수 있는가?
...
하지만 실제 온라인 환경에서는
분포가 갑자기 또는 점진적으로 변한다.
따라서 핵심 문제는 forgetting 이전에
**task segmentation과 routing**이다.
...
- 현재 expert가 더 이상 현재 입력을 잘 설명하지 못하면
  loss가 통계적으로 유의미하게 증가한다.
...
- task-agnostic setting에서도 strong baselines를 이길 수 있다.
...
- 병목은 expert보다 **routing difficulty**일 수 있다.
